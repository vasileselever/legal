export function DreptulFamiliei() {
  const services = [
    ...
    { icon: "\u{1F476}", title: "Custodia Copilului", desc: "Stabilirea autoritatii parintesti, a locuintei minorului si a programului de vizita. Apararea interesului superior al copilului." },
    ...
    { icon: "\u{1F4B5}", title: "Pensie Alimentara", desc: "Stabilirea, majorarea sau reducerea pensiei de intretinere pentru copii sau sot/sotie. Executare silita in caz de neplata." },
    { icon: "\u{1F4DD}", title: "Contracte Matrimoniale", desc: "Redactarea conventiilor matrimoniale inainte sau dupa casatorie. Regimul separatiei de bunuri, comunitatii conventionale." },
    { icon: "\u{2764}\uFE0F", title: "Adoptie", desc: "Asistenta juridica in procedura de adoptie nationala. Reprezentare in fata instantelor si autoritatilor competente." },
    { icon: "\u{1F6E1}\uFE0F", title: "Violenta Domestica", desc: "Obtinerea ordinului de protectie, asistenta victimelor violentei domestice si reprezentare in procesele penale conexe." },
    { icon: "\u{1F4D1}", title: "Tutela si Curatela", desc: "Instituirea tutelei pentru minori lipsiti de ocrotire parinteasca si a curatelei pentru persoane cu capacitate limitata." },
  ];
  ...
  return (
    <div style={{maxWidth:"900px",margin:"0 auto",padding:"2rem",fontFamily:"sans-serif"}}>
      ...
      <div style={{background:"white",padding:"2rem",borderRadius:"8px",boxShadow:"0 2px 8px rgba(0,0,0,0.1)",marginBottom:"2rem"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:"1rem"}}>
          {faqs.map((faq,i)=>(<div key={i} style={{padding:"1.25rem",background:"#f9f9f9",borderRadius:"8px",borderLeft:"4px solid #7b1fa2"}}><h4 style={{color:"#7b1fa2",margin:"0 0 0.5rem"}}>{"\u{1F4AC}"} {faq.q}</h4><p style={{color:"#555",margin:0,lineHeight:1.6}}>{faq.a}</p></div>))}
        </div>
      </div>
